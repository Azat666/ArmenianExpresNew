...
public class ProduktAdapter extends RecyclerView.Adapter<ProduktAdapter.ViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(final ProduktAdapter.ViewHolder holder, final int position) {
        holder.textType.setText(searchList.get(position).getType());
        holder.textTitle.setText(searchList.get(position).getTitle());
        holder.imageId.setImageResource(searchList.get(position).getImageId()[0]);
        if (searchList.get(position).isliked) {
            holder.buttonFavorit.setImageResource(R.drawable.ic_favorite);
        }
        if (searchList.get(position).isToBasket) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            buttonFavorit.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (!searchList.get(getAdapterPosition()).isliked) {
                        ...
                        searchList.get(getAdapterPosition()).isliked = true;
                    } else {
                        buttonFavorit.setImageResource(R.drawable.ic_favorite_border);
                        searchList.get(getAdapterPosition()).isliked = false;
                    }
                }
            });
            buttonCart.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (!searchList.get(getAdapterPosition()).isToBasket) {
                        searchList.get(getAdapterPosition()).isToBasket = true;
                        buttonCart.setImageResource(R.drawable.ic_add_shopping_);
                    } else {
                        searchList.get(getAdapterPosition()).isToBasket = false;
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    intent.putExtra(" MY_KEY ", getAdapterPosition());
                    UserProvider.position = getAdapterPosition();
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected void publishResults(CharSequence constraint, FilterResults filterResults) {
                searchList = (ArrayList<ProduktModel>) filterResults.values;
